...
crianca *retiraCrianca(cabeca **cabe, crianca **cria){
	...
	int aux = apaga->valor, aux2 = aux, cont = 5;
	char nome[30];
...
crianca *desaloca(cabeca **cabe, crianca **cria){
	...
	if(aux == (*cabe)->prim){
		printf("DESALOC PRIM: %s %d\n", (*cria)->nome, (*cria)->valor);
		(*cabe)->prim = aux->prox;
		aux = (*cria)->prox;
		free(*cria);
		*cria = NULL;
